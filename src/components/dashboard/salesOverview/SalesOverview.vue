<script setup lang="ts">
  ...
  import GoogleChart, { renData } from './Gchart';
  const elementVisible = ref(false);

  const datas = ref(renData());

  const x = () => (datas.value = renData());
...
<template>
  ...
  <v-card>
    <v-card-text>
      ...
      <div class="mt-5" v-if="elementVisible"><GoogleChart :datas="datas" /></div>
    </v-card-text>
    <button @click="x"> xx </button>
  </v-card>
</template>
